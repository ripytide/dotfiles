...
vim.lsp.handlers["textDocument/hover"] =
    vim.lsp.with(vim.lsp.handlers.hover,
                 {border = border_style, title = "hover"})
...
local function normal_lsp_mappings(bufnr)
    ...
    vim.keymap.set("n", "gj", function()
        vim.diagnostic.goto_next({float = {border = border_style}})
    end, opts)
    vim.keymap.set("n", "gk", function()
        vim.diagnostic.goto_prev({float = {border = border_style}})
    end, opts)
    ...
    vim.keymap.set("n", "<leader>f",
                   function() vim.lsp.buf.format({async = false}) end, opts)
...
require("mason-lspconfig").setup_handlers {
    ...
    function(server_name) -- default handler (optional)
        require("lspconfig")[server_name].setup {
            ...
            on_attach = function(_, bufnr) normal_lsp_mappings(bufnr) end
        }
    end
